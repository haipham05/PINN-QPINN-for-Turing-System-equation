...
Initial Conditions:
    u(x, 0) = (a+b) + 0.01*sin(2πx)
    v(x, 0) = b/(a+b)² + 0.01*cos(2πx)
...
def generate_reference_solution(config, save_path="schnakenberg_reference_solution.npy"):
    ...
    u0 = (config.A + config.B) + 0.01 * np.sin(2 * np.pi * x)
    v0 = config.B / (config.A + config.B)**2 + 0.01 * np.cos(2 * np.pi * x)
    ...
    def rd_rhs(t, y):
        """RHS for Schnakenberg system with Dirichlet BC enforcement"""
        u = y[:Nx]
        v = y[Nx:]
...
class Schnakenberg1DQPINNTrainer:
    ...
    def _setup_collocation_points(self):
        ...
        self.u0_ic = (self.config.A + self.config.B) + 0.01 * np.sin(2 * np.pi * x_ic_np)
        self.v0_ic = self.config.B / (self.config.A + self.config.B)**2 + 0.01 * np.cos(2 * np.pi * x_ic_np)
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            residual_u = du_dt - self.config.D_U * d2u_dx2 - (self.config.A - u + u2v)
            residual_v = dv_dt - self.config.D_V * d2v_dx2 - (self.config.B - u2v)
...
def main():
    ...
    print("\n" + "="*80)
    print("SCHNAKENBERG 1D QPINN TRAINING")
    print("="*80)
    ...
    ref_path = os.path.join(config.BASE_DIR, "schnakenberg_reference_solution.npy")
    if os.path.exists(ref_path):
        loaded = np.load(ref_path, allow_pickle=True)[()]
