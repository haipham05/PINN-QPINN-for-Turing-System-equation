"""
Gierer-Meinhardt 1D QPINN - Inference Script
Evaluates trained models on dense grid and compares with reference solution
...
class Config:
    ...
    X_EVAL_POINTS = 32      # Number of x points for evaluation
    T_MIN = 0.0
    T_MAX = 1.0
    X_MIN = 0.0
    X_MAX = 1.0
...
class GiererMeinhardtInferenceEngine:
    """Inference engine for Gierer-Meinhardt equation"""
    
    def __init__(self, device):
        self.device = device
        self.T_MIN, self.T_MAX = 0.0, 1.0
        self.X_MIN, self.X_MAX = 0.0, 1.0
        self.domain_min = torch.tensor([self.T_MIN, self.X_MIN], device=device)
        self.domain_max = torch.tensor([self.T_MAX, self.X_MAX], device=device)
        self._load_reference_solution()
    
    def _load_reference_solution(self):
        """Load reference solution interpolator"""
        ref_path = os.path.join("result", "gierer_meinhardt_reference_solution.npy")
    ...
    def infer_fnn_basis(self, points):
        """Inference for FNN-TE-QPINN model"""
        x_rescaled = 2.0 * (points - self.domain_min) / (self.domain_max - self.domain_min) - 1.0
    ...
    def infer_qnn(self, points):
        """Inference for QNN-TE-QPINN model"""
        x_rescaled = 2.0 * (points - self.domain_min) / (self.domain_max - self.domain_min) - 1.0
    ...
    def evaluate(self, t_eval, x_eval, model_type):
        """Evaluate model on grid"""
        # Create dense evaluation grid
        t_grid, x_grid = np.meshgrid(t_eval, x_eval, indexing='ij')
        points = np.column_stack([t_grid.ravel(), x_grid.ravel()])
        points_torch = torch.tensor(points, dtype=torch.float32, device=self.device)
        
        # Load model
        if model_type == "PINN":
            self.pinn = FNNBasisNet(2, 20, 2, input_dim=2).to(self.device)
            self.pinn.load_state_dict(torch.load("result/gierer_meinhardt_1d_pinn.pt"))
            self.pinn.eval()
            with torch.no_grad():
                pred = self.infer_pinn(points_torch)
        elif model_type == "FNN":
            data = np.load("result/gierer_meinhardt_1d_fnn_basis.npy", allow_pickle=True)[()]
            self.theta = data['theta']
            self.basis_net = FNNBasisNet(2, 20, 4, input_dim=2).to(self.device)
            self.basis_net.load_state_dict(data['basis_net'])
            self.basis_net.eval()
            with torch.no_grad():
                pred = self.infer_fnn_basis(points_torch)
        else:
            data = np.load("result/gierer_meinhardt_1d_qnn.npy", allow_pickle=True)[()]
            self.theta = data['theta']
            self.qnn_embedding = QNNEmbedding(4, 2, 4, input_dim=2).to(self.device)
        ...
        # Reshape predictions
        u_pred = pred[:, 0].reshape(t_grid.shape)
        v_pred = pred[:, 1].reshape(t_grid.shape)
        
        # Get reference solution
        u_ref = np.array([[self.interp_u([t, x]).squeeze() for x in x_eval] for t in t_eval])
        v_ref = np.array([[self.interp_v([t, x]).squeeze() for x in x_eval] for t in t_eval])
...
if __name__ == "__main__":
    ...
    engine = GiererMeinhardtInferenceEngine(device)
    ...
    print("=" * 80)
    print("GIERER-MEINHARDT 1D INFERENCE RESULTS")
    print("=" * 80)
    
    for model_type in ["PINN", "FNN", "QNN"]:
        try:
            u, v, u_ref, v_ref = engine.evaluate(t_eval, x_eval, model_type)
