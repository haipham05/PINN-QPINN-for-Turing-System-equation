"""
Gierer-Meinhardt 1D QPINN - Training Script
...
Domain: t ∈ [0, 1], x ∈ [0, 1]
PDE System (Gierer-Meinhardt):
    ∂u/∂t = D_u ∂²u/∂x² + (a*u²/v) - u + ρ
    ∂v/∂t = D_v ∂²v/∂x² + a*u² - v

Initial Conditions:
    u(x, 0) = 0.5 + 0.1*sin(2πx)
    v(x, 0) = 0.5 + 0.1*sin(2πx)
...
def generate_reference_solution(config, save_path="gierer_meinhardt_reference_solution.npy"):
    ...
    print("=== Generating 1D Gierer-Meinhardt Reference Solution ===")
    ...
    def rd_rhs(t, y):
        """RHS for Gierer-Meinhardt system with Dirichlet BC enforcement"""
        u = y[:Nx]
        v = y[Nx:]
...
def plot_reference_solution(t, x, u_sol, v_sol, save_dir="result"):
    ...
    plt.suptitle('Plot 2: Reference Solution (RK45) - 1D Gierer-Meinhardt', 
...
def plot_embedding_results(trainer, save_dir="result"):
    ...
    plt.suptitle(r"Plot 4: Trainable Embedding Functions: $\phi(x) \cdot x$ (Gierer-Meinhardt 1D)", 
...
class GiererMeinhardt1DQPINNTrainer:
    ...
    def _setup_collocation_points(self):
        ...
        self.u0_ic = 0.5 + 0.1 * np.sin(2.0 * np.pi * x_ic_np)
        self.v0_ic = 0.5 + 0.1 * np.sin(2.0 * np.pi * x_ic_np)
    ...
    def _load_reference_solution(self):
        ...
        ref_path = os.path.join(self.config.BASE_DIR, "gierer_meinhardt_reference_solution.npy")
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            residual_u = du_dt - self.config.D_U * d2u_dx2 - (self.config.A * (u**2) / (v + 1e-6)) + u - self.config.RHO
            residual_v = dv_dt - self.config.D_V * d2v_dx2 - self.config.A * (u**2) + v
...
def main():
    ...
    print("\n" + "="*80)
    print("GIERER-MEINHARDT 1D QPINN TRAINING")
    print("="*80)
    ...
    # Initialize trainer
    trainer = GiererMeinhardt1DQPINNTrainer(config, device)
    
    # === Plot 1: Collocation Points ===
    plot_collocation_points_1d(config, config.OUTPUT_DIR)
    
    # === Plot 2: Reference Solution ===
    ref_path = os.path.join(config.BASE_DIR, "gierer_meinhardt_reference_solution.npy")
    if os.path.exists(ref_path):
        loaded = np.load(ref_path, allow_pickle=True)[()]
