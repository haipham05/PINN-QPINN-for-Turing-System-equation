...
def generate_reference_solution(config, save_path="gierer_meinhardt_reference_solution.npy"):
    ...
    # Initial conditions for Gierer-Meinhardt
...
class GiererMeinhardt1DQPINNTrainer:
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            residual_u = du_dt - self.config.D_U * d2u_dx2 - (self.config.A * u**2 / v) + u - self.config.RHO
    ...
    def save_model(self, embedding_type, method_name):
        ...
        if embedding_type == "FNN_BASIS":
            ...
            path = os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_1d_fnn_basis.npy")
        elif embedding_type == "QNN":
            model_state = {
                'theta': self.theta.detach().cpu().numpy(),
                'qnn_embedding': self.qnn_embedding.state_dict()
            }
            path = os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_1d_qnn.npy")
        else:
            torch.save(self.pinn.state_dict(), os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_1d_pinn.pt"))
            return
