"""
Gierer-Meinhardt 2D QPINN - Inference Script
...
class GiererMeinhardt2DInferenceEngine:
    """Inference engine for 2D Gierer-Meinhardt equation"""
    ...
    def _load_reference_solution(self):
        ...
        ref_path = os.path.join("result", "gierer_meinhardt_reference_solution_2d.npy")
    ...
    def evaluate(self, t_eval, x_eval, y_eval, model_type):
        ...
        if model_type == "PINN":
            ...
            self.pinn.load_state_dict(torch.load("result/gierer_meinhardt_2d_pinn.pt"))
            self.pinn.eval()
            with torch.no_grad():
                pred = self.infer_pinn(points_torch)
        elif model_type == "FNN":
            data = np.load("result/gierer_meinhardt_2d_fnn_basis.npy", allow_pickle=True)[()]
            ...
            data = np.load("result/gierer_meinhardt_2d_qnn.npy", allow_pickle=True)[()]
            self.theta = data['theta']
...
if __name__ == "__main__":
    ...
    engine = GiererMeinhardt2DInferenceEngine(device)
    ...
    print("=" * 90)
    print("GIERER-MEINHARDT 2D INFERENCE RESULTS")
    print("=" * 90)
