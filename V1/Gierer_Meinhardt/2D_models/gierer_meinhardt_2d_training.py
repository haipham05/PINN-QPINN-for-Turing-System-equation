...
class Config:
    """Configuration for Gierer-Meinhardt 2D QPINN"""
...
def generate_2d_reference_solution(config, save_path="gierer_meinhardt_reference_solution_2d.npy"):
    """Generate 2D reference solution"""
    
    print("=== Generating 2D Gierer-Meinhardt Reference Solution ===")
    ...
    def rd_rhs(t, y):
        """RHS for 2D Gierer-Meinhardt system"""
        u = y[:Nx*Ny]
        v = y[Nx*Ny:]
        lapU = laplacian_2d(u, dx, dy, Nx, Ny)
        lapV = laplacian_2d(v, dx, dy, Nx, Ny)
        
        du = config.D_U * lapU + (config.A * u**2 / v) - u + config.RHO
...
class GiererMeinhardt2DQPINNTrainer:
    """Trainer for 2D Gierer-Meinhardt QPINN"""
    ...
    def _load_reference_solution(self):
        ...
        ref_path = os.path.join(self.config.BASE_DIR, "gierer_meinhardt_reference_solution_2d.npy")
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            # Gierer-Meinhardt PDE residuals
            residual_u = du_dt - self.config.D_U * (d2u_dx2 + d2u_dy2) - (self.config.A * u**2 / v) + u - self.config.RHO
    ...
    def save_model(self, embedding_type, method_name):
        ...
        if embedding_type == "FNN_BASIS":
            ...
            path = os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_2d_fnn_basis.npy")
        elif embedding_type == "QNN":
            model_state = {
                'theta': self.theta.detach().cpu().numpy(),
                'qnn_embedding': self.qnn_embedding.state_dict()
            }
            path = os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_2d_qnn.npy")
        else:
            torch.save(self.pinn.state_dict(), os.path.join(self.config.OUTPUT_DIR, f"gierer_meinhardt_2d_pinn.pt"))
            return
