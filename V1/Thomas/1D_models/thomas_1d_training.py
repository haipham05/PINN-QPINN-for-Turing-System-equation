"""
Thomas 1D QPINN - Training Script
...
Domain: t ∈ [0, 1], x ∈ [0, 1]
PDE System (Thomas):
    ∂u/∂t = D_u ∂²u/∂x² + a - u - ρuv/(1 + u + Ku²)
    ∂v/∂t = D_v ∂²v/∂x² + α(b - v) - ρuv/(1 + u + Ku²)

Initial Conditions:
    u(x, 0) = 5.0 + 0.1*sin(2πx)
    v(x, 0) = 10.0 + 0.1*cos(2πx)
...
def generate_reference_solution(config, save_path="thomas_reference_solution.npy"):
    """Generate 1D reference solution using RK45 solver"""
    
    print("=== Generating 1D Thomas Reference Solution ===")
    ...
    u0 = 5.0 + 0.1 * np.sin(2 * np.pi * x)
    v0 = 10.0 + 0.1 * np.cos(2 * np.pi * x)
    ...
    def rd_rhs(t, y):
        """RHS for Thomas system with Dirichlet BC enforcement"""
        u = y[:Nx]
        v = y[Nx:]
    ...
    print(f"Solving Thomas PDE with RK45...")
...
def plot_reference_solution(t, x, u_sol, v_sol, save_dir="result"):
    ...
    plt.suptitle('Plot 2: Reference Solution (RK45) - 1D Thomas', 
...
def plot_embedding_results(trainer, save_dir="result"):
    ...
    plt.suptitle(r"Plot 4: Trainable Embedding Functions: $\phi(x) \cdot x$ (Thomas 1D)", 
...
class Thomas1DQPINNTrainer:
    """Trainer class for Thomas 1D QPINN models"""
    ...
    def _setup_collocation_points(self):
        ...
        self.u0_ic = 5.0 + 0.1 * np.sin(2.0 * np.pi * x_ic_np)
        self.v0_ic = 10.0 + 0.1 * np.cos(2.0 * np.pi * x_ic_np)
    ...
    def _load_reference_solution(self):
        ...
        ref_path = os.path.join(self.config.BASE_DIR, "thomas_reference_solution.npy")
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            # PDE residuals
            residual_u = du_dt - self.config.D_U * d2u_dx2 - (self.config.A - u - reaction)
            residual_v = dv_dt - self.config.D_V * d2v_dx2 - (self.config.ALPHA * (self.config.B - v) - reaction)
...
class TrainingVisualizer:
    ...
    def plot_training_analysis(self, save_dir="result"):
        ...
        for embedding_type in ["NONE", "FNN_BASIS", "QNN"]:
            ...
            plt.suptitle(f'Plot 5: {method_name} Training Analysis (Thomas 1D)', fontsize=16, fontweight='bold')
            plt.tight_layout()
...
def main():
    ...
    print("\n" + "="*80)
    print("THOMAS 1D QPINN TRAINING")
    print("="*80)
    print(f"Domain: t ∈ [{config.T_MIN}, {config.T_MAX}], x ∈ [{config.X_MIN}, {config.X_MAX}]")
    print(f"Parameters: D_U={config.D_U}, D_V={config.D_V}, A={config.A}, B={config.B}, RHO={config.RHO}, ALPHA={config.ALPHA}, K={config.K}")
    print(f"Training iterations: {config.TRAINING_ITERATIONS}")
    print("="*80)
    
    # Initialize trainer
    trainer = Thomas1DQPINNTrainer(config, device)
    
    # === Plot 1: Collocation Points ===
    plot_collocation_points_1d(config, config.OUTPUT_DIR)
    
    # === Plot 2: Reference Solution ===
    ref_path = os.path.join(config.BASE_DIR, "thomas_reference_solution.npy")
    if os.path.exists(ref_path):
        loaded = np.load(ref_path, allow_pickle=True)[()]
