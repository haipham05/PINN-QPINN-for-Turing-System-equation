...
class Thomas1DQPINNTrainer:
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            residual_u = du_dt - self.config.D_U * d2u_dx2 + u - self.config.A * (v ** 2)
            residual_v = dv_dt - self.config.D_V * d2v_dx2 + v - self.config.B * u
    ...
    def save_model(self, embedding_type, method_name):
        ...
        if embedding_type == "FNN_BASIS":
            ...
            path = os.path.join(self.config.OUTPUT_DIR, f"thomas_1d_fnn_basis.npy")
        elif embedding_type == "QNN":
            model_state = {
                'theta': self.theta.detach().cpu().numpy(),
                'qnn_embedding': self.qnn_embedding.state_dict()
            }
            path = os.path.join(self.config.OUTPUT_DIR, f"thomas_1d_qnn.npy")
        else:
            torch.save(self.pinn.state_dict(), os.path.join(self.config.OUTPUT_DIR, f"thomas_1d_pinn.pt"))
            return
