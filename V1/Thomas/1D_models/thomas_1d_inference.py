...
class ThomasInferenceEngine:
    """Inference engine for Thomas equation"""
    ...
    def _load_reference_solution(self):
        ...
        ref_path = os.path.join("result", "thomas_reference_solution.npy")
    ...
    def evaluate(self, t_eval, x_eval, model_type):
        ...
        if model_type == "PINN":
            ...
            self.pinn.load_state_dict(torch.load("result/thomas_1d_pinn.pt"))
            self.pinn.eval()
            with torch.no_grad():
                pred = self.infer_pinn(points_torch)
        elif model_type == "FNN":
            data = np.load("result/thomas_1d_fnn_basis.npy", allow_pickle=True)[()]
            ...
            data = np.load("result/thomas_1d_qnn.npy", allow_pickle=True)[()]
            self.theta = data['theta']
...
if __name__ == "__main__":
    ...
    engine = ThomasInferenceEngine(device)
    ...
    print("=" * 80)
    print("THOMAS 1D INFERENCE RESULTS")
    print("=" * 80)
