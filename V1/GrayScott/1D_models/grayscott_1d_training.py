...
def generate_reference_solution(config, save_path="grayscott_reference_solution.npy"):
    ...
    # Initial conditions for Gray-Scott
    u0 = np.ones(Nx)
    v0 = np.zeros(Nx)
    mid = Nx // 2
...
class GrayScott1DQPINNTrainer:
    ...
    def _setup_collocation_points(self):
        ...
        self.u0_ic = np.ones(Nx)
        self.v0_ic = np.zeros(Nx)
        mid = Nx // 2
        self.v0_ic[mid-5:mid+5] = 0.5
    ...
    def model(self, x):
        """Forward pass through the current model (no postprocessing rescaling)"""
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            residual_u = du_dt - self.config.D_U * d2u_dx2 + reaction - self.config.F * (1.0 - u)
            residual_v = dv_dt - self.config.D_V * d2v_dx2 - reaction + (self.config.K + self.config.F) * v
    ...
    def save_model(self, embedding_type, method_name):
        ...
        if embedding_type == "FNN_BASIS":
            ...
            path = os.path.join(self.config.OUTPUT_DIR, f"grayscott_1d_fnn_basis.npy")
        elif embedding_type == "QNN":
            model_state = {
                'theta': self.theta.detach().cpu().numpy(),
                'qnn_embedding': self.qnn_embedding.state_dict()
            }
            path = os.path.join(self.config.OUTPUT_DIR, f"grayscott_1d_qnn.npy")
        else:  # PINN
            torch.save(self.pinn.state_dict(), os.path.join(self.config.OUTPUT_DIR, f"grayscott_1d_pinn.pt"))
            return
