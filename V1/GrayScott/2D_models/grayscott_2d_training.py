...
PDE System (Gray-Scott 2D):
    ∂u/∂t = D_u (∂²u/∂x² + ∂²u/∂y²) - uv² + F(1-u)
    ∂v/∂t = D_v (∂²v/∂x² + ∂²v/∂y²) + uv² - (F+k)v

Initial Conditions:
    u(x, y, 0) = 1 - 0.5*sin²(πx)sin²(πy)
    v(x, y, 0) = 0.25*sin²(πx)sin²(πy)

Boundary Conditions (Dirichlet):
    u = 1, v = 0 on all boundaries
...
def plot_reference_solution(t, x, y, u_sol, v_sol, save_dir="result"):
    ...
    plt.suptitle('Plot 2: Reference Solution (RK45) - 2D Brusselator', 
...
def generate_reference_solution(config, save_path="grayscott_3d_reference_solution.npy"):
    ...
    X, Y = np.meshgrid(x, y, indexing='ij')
    u0 = 1.0 - 0.5 * (np.sin(np.pi * X) ** 2) * (np.sin(np.pi * Y) ** 2)
    v0 = 0.25 * (np.sin(np.pi * X) ** 2) * (np.sin(np.pi * Y) ** 2)
    ...
    def rd_rhs_3d(t, y_flat):
        """RHS for 2D Gray-Scott system with Dirichlet BC enforcement"""
        u = y_flat[:Nx*Ny].reshape(Nx, Ny)
        v = y_flat[Nx*Ny:].reshape(Nx, Ny)
        
        # Enforce Dirichlet BC: u=1, v=0 at boundaries
        ...
        uv2 = u * (v**2)
        du = config.D_U * lapU - uv2 + config.F * (1.0 - u)
        dv = config.D_V * lapV + uv2 - (config.F + config.K) * v
...
class GrayScott2DQPINNTrainer:
    """Trainer class for Gray-Scott 2D (2D spatial + time) QPINN models"""
    ...
    def _setup_collocation_points(self):
        ...
        self.u0_ic = 1.0 - 0.5 * (np.sin(np.pi * X_ic) ** 2) * (np.sin(np.pi * Y_ic) ** 2)
        self.v0_ic = 0.25 * (np.sin(np.pi * X_ic) ** 2) * (np.sin(np.pi * Y_ic) ** 2)
    ...
    def _create_loss_functions(self):
        ...
        def pde_loss():
            ...
            # PDE residuals
            residual_u = du_dt - self.config.D_U * laplacian_u + uv2 - self.config.F * (1.0 - u)
            residual_v = dv_dt - self.config.D_V * laplacian_v - uv2 + (self.config.F + self.config.K) * v
    ...
    def train_model(self, embedding_type, iterations):
        ...
        for epoch in range(iterations):
            ...
            uv2 = u * (v ** 2)
            residual_u = du_dt - self.config.D_U * (d2u_dx2 + d2u_dy2) + uv2 - self.config.F * (1.0 - u)
            residual_v = dv_dt - self.config.D_V * (d2v_dx2 + d2v_dy2) - uv2 + (self.config.F + self.config.K) * v
...
def main():
    ...
    print("\n" + "="*80)
    print("GRAY-SCOTT 2D QPINN TRAINING (2D Spatial + Time)")
    print("="*80)
    ...
    ref_path = os.path.join(config.BASE_DIR, "grayscott_3d_reference_solution.npy")
